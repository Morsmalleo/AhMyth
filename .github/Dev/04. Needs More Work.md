## Updated `getLauncherActivity` & `getLauncherPath` functions 
> These updated functions contain a 99% Accuracy rating when it comes to their combined usage of locating either an APK file's Main Application Class or its Main Launcher Activity class according to chatGPT.
> Just needs its logs cleaned up and errors that arise written to error log files

<details>
    <summary>updated "getLauncherActivity" & "getLauncherPath" functions + updated usage</summary>
    <br>
...
function getLauncherActivity(manifest) {
    ...
    if (mainApplicationClassName) {
        if (!isSystemClass(mainApplicationClassName)) {
            ...
            return processClassName(mainApplicationClassName); // Only process class name
        } else {
            ...
            if (intentFilter) {
                ...
                if (isLauncherAlias) {
                    ...
                    if (targetActivityName && targetActivityName.trim() !== '') {
                        // Process the target activity class (likely adds '.smali' extension)
...
function getLauncherPath(launcherActivity, apkFolder, callback) {
    ...
    const stream = readdirp(apkFolder, {
        fileFilter: (entry) => {
            // Allow partial matching of the launcher activity name and ignore inner classes
    ...
    stream
        .on('data', (entry) => {
            if (!found) {
                // Ensure only the first match is processed
                found = true;

                // Sanitize launcherPath just like the old version
        ...
        .on('end',
            () => {
                if (!found) {
                    const errorMessage = '[x] Unable to Locate the Hookable Main Class File!\n[x] Please Use the "On Boot" Method!';
